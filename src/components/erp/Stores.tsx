...
import { useStore, setState, logActivity, uid, addToStock, type InwardEntry } from "@/lib/erp-store";
import { Card, Pill, Button, Field, Input, Select, Modal, Table, Empty, toast } from "./ui";
import { fmtDate, fmtINR, fmtNum, todayISO, isoToInputDate } from "@/lib/format";

export function Stores({ view, readOnly = false }: { view: "inward" | "stock" | "issue"; readOnly?: boolean }) {
...
function Inward({ readOnly }: { readOnly: boolean }) {
  const inwards = useStore(s => s.inwards);
  const [show, setShow] = useState(false);
  const [qcEntry, setQcEntry] = useState<InwardEntry | null>(null);
...
function NewInwardModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [f, setF] = useState({
    ...
    qty: 100, coils: 1, unit: "m" as "m" | "nos", rate: rates[0].rate, woNo: orders[0]?.woNo ?? "",
  });
  ...
  function save() {
    ...
    setState(s => {
      ...
      if (o && o.stage === "Purchase Received") o.stage = "Incoming QC";
      return s;
    });
    logActivity("Inventory", `GRN ${f.grnNo} received — ${f.partyName} (${f.itemDescription}, ${fmtNum(f.qty)} ${f.unit})`);
  ...
  return (
    <Modal open={open} onClose={onClose} title="New inward entry" maxW="max-w-3xl">
      <div className="grid grid-cols-3 gap-4">
        ...
        <Field label="Unit"><Select value={f.unit} onChange={e => setF({ ...f, unit: e.target.value as "m" | "nos" })}><option value="m">metres</option><option value="nos">nos</option></Select></Field>
...
function QCModal({ entry, onClose }: { entry: InwardEntry | null; onClose: () => void }) {
  const [rows, setRows] = useState(() => IQP_ROWS.map(r => ({ ...r })));
  const [checkedBy, setCheckedBy] = useState("Sunil Yadav");
  const [disp, setDisp] = useState<"Accept" | "Rework" | "Scrap" | "Return">("Accept");
  ...
  return (
    <Modal open={!!entry} onClose={onClose} title={`Incoming QC — ${e0.itemDescription}`} maxW="max-w-4xl">
      <div className="grid md:grid-cols-4 gap-4 mb-5">
        <Info label="Material" value={e0.itemDescription} />
        <Info label="GRN" value={e0.grnNo} />
        <Info label="Lot" value={e0.lotNo} />
        <Info label="Party" value={e0.partyName} />
      </div>
...
function IssueToJob({ readOnly }: { readOnly: boolean }) {
  const stock = useStore(s => s.stock);
  const orders = useStore(s => s.orders);
  const issues = useStore(s => s.issues);
  const [item, setItem] = useState(stock[0]?.description ?? "");
